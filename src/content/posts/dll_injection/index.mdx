...
This page aims to explain how I used Visual C++ to write a DLL injector tool for :link[RuneScape]{id="http://runescape.com/"}'s new NXT Client as well as a DLL that hooks into RuneScape's OpenGL draw calls to render a full :link[Dear ImGui]{#ocornut/imgui} overlay on top of it.

<Figure src={import("assets/cover.png")} alt="RuneScape with ImGui Overlay"/>
...
This part is done in preparation for the next step. It's allocating memory in the remote process and places the DLL string inside of it.
...
We simply let the Game draw all its content and when it calls the function to end the current frame, we draw our overlay on top before calling the actual end frame function.

<NotePanel type="info" title="Graphics APIs">
  An easy way to find out what the Game you want to hook uses, is to load the Game executable into Ghidra and checking its imports in the
  Symbol Tree.
</NotePanel>
<Figure src={import("assets/ghidra_imports.png")} alt="Ghidra Imports"/>
...
<Figure src={import("assets/swapBuffers_pre.png")} alt="Pre Patching"/>

<Figure src={import("assets/swapBuffers_post.png")} alt="Post Patching"/>

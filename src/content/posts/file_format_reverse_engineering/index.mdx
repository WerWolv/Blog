...
If we want to be extra fancy and make sure that we only load files that are actually compatible with our pattern, we can easily assert on this. In the Pattern Language, we can have conditions and function calls intertwined with our type definitions which makes things like this possible:

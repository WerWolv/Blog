...
```txt wrap=false title="SaveSlot2"
...
We can also write type aliases with the `using` keyword to make the code in our pattern resemble the types used in the real code even more closely.
...
struct FezSaveFile {
  ...
  long someStaticValue; // Could be a save file version number?
  long creationTime;
  bool finished32;
  bool finished64;
  bool hasFpView;

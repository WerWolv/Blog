...
The first thing we need is the save file. I downloaded the game from Steam, started it and played for a little bit until it saved. Then I went looking through my filesystem and found the save file under `/home/werwolv/.local/share/FEZ/SaveSlot2`. On Windows, it will be elsewhere.
...
Next, in the save file generation code, we see `writer.Write(DateTime.Now.ToFileTime());`, which writes the current timestamp as a :link[Windows File Time]{id="https://learn.microsoft.com/en-us/dotnet/api/system.datetime.tofiletime?view=net-10.0#remarks"} to the output. As seen in the Remarks section of the docs, this is simply a 64-bit value that represents the number of 100 ns intervals that have passed since the year of our lord 1601 A.D. We could, of course, properly decode this value and everything, but it's not really all that interesting, so we can simply place a `u64` in its place in the Pattern to read it.
...
If you have any questions about the process, ImHex or the Pattern Language itself, please reach out on the :link[ImHex Discord Server]{id="https://discord.gg/X63jZ36xBY"}, via :link[Discord DMs @werwolv]{id="https://discord.com/users/181738643008782346"} or by [email at `hey@werwolv.net`](mailto:hey@werwolv.net).

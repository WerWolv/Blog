...
For the Boot ROM to recognize and load the FSBL (or in fact any binary), a special format is used. It is described as the [STM32 header for binary files](https://wiki.st.com/stm32mpu/wiki/STM32_header_for_binary_files) and consists of a 256 bytes long header followed by the binary data to load.

...
<NotePanel type="hint" title="Function names">
    Note the triple-underscore here, the linker prepends `__wrap_` and `__real_` to the function name and `__cxa_throw` already starts with two underscores...
</NotePanel>
...
extern "C" auto __wrap___cxa_throw(
  ...
  //   Since the real __cxa_throw hasn't been called yet, this gives
  //   us the index we want to store the exception at in the global object
  auto exception_index = std::uncaught_exceptions();

  // If there's still some space on the exception stack,
  //   capture a new stacktrace and add it to the stack
  if (exception_index < ssize_t(s_stacktraces.size())) {
    ...
    s_stacktraces[exception_index] = std::stacktrace::current(1);
  }
...
auto print_stacktrace() -> void {
  // This function returns the number of exceptions currently in-flight.
  //   Since we're calling it from a catch-handler, that exception was already
  //   caught so it returns the number of exceptions in flight - 1.
  //   This conveniently is the index into the s_stacktraces array
  auto last_exception_index = std::uncaught_exceptions();
  
  if (last_exception_index >= s_stacktraces.size()) {
    std::println("Too many active exceptions");
    return;
  }

  std::print("Stacktrace:\n{}", s_stacktraces[last_exception_index]);
}
```
